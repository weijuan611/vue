<template>
    <page-wrap>
        ...
        <!-- 标题栏 -->
        <page-title title="访客分析-地区分析" :date="searchData.dateTime" />
        ...
        <!-- 其它的都放在这儿 -->
        <other-wrap>
          <rectangle :data="areaDataDetail" cols="7" />
        ...
        <table-wrap class="areaAnalysis_table">
          <el-form ref="form" :model="searchData" size="mini" class="withTableHeader" style="padding: 8px 0;">
              <el-form-item label="选择指标：" style="margin-left: 10px;">
                ...
                <el-button type="success" class="button" @click="onExport" style="display: none;"
                :disabled=!buttonControl.areaexport>导出表格</el-button>
              </el-form-item>
          </el-form>
          <el-table
            ref="table"
            ...
            <el-table-column type="expand">
              <template slot-scope="props">
                <div class="expandWrap" style="background-color: #fff;" v-show="props.row.name !== '不详' && props.row.name !== '全站总计'">
        ...
        methods: {
            ...
            onExport () {
                ...
                that.$ajax.post('/area/export', {"type":"1"})
                    .then(function (res) {
                        if (res.data === 'ok') {
                            window.location = 'index.php/area/export?tableDataClassify=' + that.searchData.tableDataClassify + '&start_time=' + that.searchData.dateTime[0] + '&end_time=' + that.searchData.dateTime[1] + '&tongji='+that.searchData.tongji + '&prop='+that.paginationData.prop + '&order='+that.paginationData.order;
                        }
                        that.loading = false
                    })
                    .catch(function(err) {

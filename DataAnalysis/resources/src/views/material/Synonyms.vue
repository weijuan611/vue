    ...
    <page-wrap>
                    ...
                    <el-col :span="7">
                        <div class="bg">
                            <el-form-item label="同义词：">
                                <el-input v-model="searchData.synonyms" placeholder="请输入需要查找的违禁词"></el-input>
                            </el-form-item>
                        </div>
                    </el-col>
        ...
        <table-wrap>
            <div class="withTableHeader" style="width: 99.8%;">
                <el-button type="primary" plain size="mini" @click="denyWordsBatchChangeType('1')" :disabled="!buttonControl.material_synonymsmaterial_changetype">批量激活</el-button>
                <el-button type="danger" plain size="mini" @click="denyWordsBatchChangeType('0')" :disabled="!buttonControl.material_synonymsmaterial_changetype">批量停用</el-button>
            </div>
            <el-table
                ...
                <el-table-column label="操作" header-align="center" width="110px">
                    <template slot-scope="scope">
                        <el-button type="text" plain size="mini" icon="el-icon-delete"
                                   @click="onDelete(scope.row,0)" v-show="scope.row.status == 1" :disabled="!buttonControl.material_synonymsmaterial_changetype">停用
                        </el-button>
                        <el-button type="text" plain size="mini" icon="el-icon-setting"
                                   @click="onDelete(scope.row,1)" v-show="scope.row.status == 0" :disabled="!buttonControl.material_synonymsmaterial_changetype">激活
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <synonyms-add :handle="addDialog" @callback="onAddCallback"/>
    </page-wrap>
</template>
...
<script>
    ...
    import SynonymsAdd from './SynonymsAdd.vue'
    import {mapState} from 'vuex'
    import { getDates }  from './../../assets/js/baseFunc/baseSelfFunc'

    export default {
        components: {
            PageWrap, SearchWrap, TableWrap, paginationWrap, NewPagination, SynonymsAdd
        },
        data() {
            return {
                addDialog:false,
        ...
        methods: {
            ...
            onSubmit(action = '') {
                ...
                if (!that.loading) {
                    this.$ajax.post('/material_synonyms/init', {
            ...
            selectTableRow(selection) {
                this.checkMany = selection.map(m => m.sy_id)
            },
            ...
            onDelete(row,type) {
                ...
                this.$confirm(message, '提示', {
                    ...
                    this.$ajax.post('/material_synonyms/material_changetype', {id: row.sy_id,type:type})
                        .then(function (res) {
                            if (res.data.type) {
            ...
            denyWordsBatchChangeType (type) {
                let that = this;
                this.$ajax.post('material_synonyms/material_changetype', {type:type,id:that.checkMany})
                    .then(function (res) {
                        if (res.data.type){
                            that.openEditmaterial = false
                            that.onSubmit()

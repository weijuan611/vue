...
class Spider extends Process
{
    ...
    public function parse($work)
    {
        ...
        echo 'crawler:' . $kw_id . '|' . $keyword . PHP_EOL;
        switch ($this->mode) {
            case 0:
                ...
                break;
            case 1:
                $this->process_hx($kw_id, $keyword);
                Log::write($keyword . ' houxue statistics processing has finished.');
                break;
        }
...
//        $result = preg_match_all("/(http|ftp|https):[\/]{2}[a-z]+[.]{1}[a-z\d\-]+[.]{1}[a-z\d]*[\/]*[A-Za-z\d]*[\/]*[A-Za-z\d]*/", $res, $matches);
     ...
     * 厚学关键词统计计算，低频分开算
     */
    public function process_hx($kw_id_in, $keyword_in)
    {
        echo '---厚学关键词统计计算开始---' . PHP_EOL;
        //===step 2: 厚学关键词
        ...
        if (empty($result_hx)) {
            try {
                Db::table('keywords')->where('kw_id', '=', $kw_id_in)->update($data_arr);
            } catch (\Exception $e) {
                return ['type' => 'error', 'msg' => $e->getLine() . '数据库操作失败!' . $e->getMessage() . $e->getTraceAsString()];
            }
        } else {
                ...
                try {
                    ...
                    if (!empty($kwd_id_insert)) {
                        ...
                        foreach ($kwd_id_insert as $k1 => $kwd_id) {
                            $temp = Db::table("keywords_detail kd")
                                ->join("keywords k", "kd.kw_id  = k.kw_id", 'LEFT')
        ...
        // 标记为计算完成
        try {
            Db::table('keywords')->where('kw_id', '=', $kw_id_in)->update(['status_hx' => 2]);
        } catch (Exception $e) {
            return ['type' => 'error', 'msg' => $e->getLine() . '数据库操作失败!' . $e->getMessage() . $e->getTraceAsString()];
        }
        Log::write('关键词厚学统计处理完成');
        return true;
    }
}
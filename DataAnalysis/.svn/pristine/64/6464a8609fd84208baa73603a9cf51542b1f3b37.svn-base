...
class Mlexicon extends Base
{
    ...
    public function keywords_add()
    {
        ...
        try{
            ...
            Db::table("keywords_check")->insert(['kw_id'=>$kw_id,'status'=>"0","type"=>"0"]);
            $data = [];
            $num = count($tags);
            for ($i=0;$i<$num;$i++) {
                $data[$i]['kw_id'] = $kw_id;
            ...
            Db::table("keywords_uncheck")->insert(['kw_id'=>$kw_id,'c_id'=>$class,"tags"=>$info,"createtime"=>date("Y-m-d H:i:s"),"type"=>0]);
            Db::commit();
    ...
    public function keywords_edit()
    {
        ...
        $keyword = empty($request['keyword'])?"":trim($request['keyword']);
        $class = empty($request['class'])?"":$request['class'];
        $tags = empty($request['tags'])?[]:array_unique($request['tags']);
        $userid = Session::get("org_user_id");
        $time = date("Y-m-d H:i:s");
        if ($id == "")
        ...
        try{
            ...
            if (empty($result_check)) {
                Db::table("keywords_check")->insert(['kw_id'=>$id,'status'=>"0","type"=>"1"]);
            } else {
                Db::table("keywords_check")->where("kw_id","=",$id)->update(['status'=>"0","type"=>"1"]);
            }
            Db::table("keywords_uncheck")->insert(['kw_id'=>$id,'c_id'=>$class,"tags"=>$info,"createtime"=>date("Y-m-d H:i:s"),"type"=>1]);
            Db::commit();
    ...
    public static function keywords_delete()
    {
        ...
        try{
            ...
            if($num>0){
                Db::table('keywords_check')->where('kw_id','=',$id)->update(['status'=>"0","type"=>"3"]);
            }else{
                Db::table("keywords_check")->insert(['kw_id'=>$id,'status'=>"0","type"=>"3"]);
            }
            Db::table("keywords_uncheck")->insert(['kw_id'=>$id,"createtime"=>date("Y-m-d H:i:s"),"type"=>3]);
            Db::commit();
...
//            if (!empty($type_1)){
//                $query=$this->createquery($query,'kl.l_id',$search_data['value9']);
//            }
//            if (!empty($type_0)){
//                $query=$this->createquery($query,'kl.l_id',$search_data['value9']);
//            }

//            $query->join('keywords_label kl','k.kw_id = kl.kw_id','left');
...
//            $query_label->delete();
            $idnum = count($idarr);
            $tagsnum = count($tags);
            $data = $uncheckarr = $checkarr = $listarr = [];
            for ($i=0;$i<$idnum;$i++) {
                for ($j=0;$j<$tagsnum;$j++) {
                    $k = $i*$tagsnum+$j;
            ...
            foreach ($idarr as $key=>$val) {
                ...
                $checkarr[$key]['type'] = "1";
                $uncheckarr[$key]['kw_id'] = $val;
                $uncheckarr[$key]['c_id'] = $class;
                $uncheckarr[$key]['tags'] = serialize($listarr[$val]);
                $uncheckarr[$key]['createtime'] = date("Y-m-d H:i:s");
                $uncheckarr[$key]['type'] = 1;
            }
            Utility::keywordsLog($idarr,1,'批量修改关键词');
            Db::table("keywords_check")->insertAll($checkarr);
            Db::table("keywords_uncheck")->insertAll($uncheckarr);
            ...
            try {
                foreach ($not_in as $item) { //循环获取各字段值
                   ...
                   $id= Db::table("keywords")->insertGetId($arr);
                    Db::table("keywords_uncheck")->insert(['kw_id'=>$id,'c_id'=>$class_id,"createtime"=>date("Y-m-d H:i:s"),"type"=>0]);
                   Db::table('keywords_check')->insert(['kw_id'=>$id,'user_id'=>Session('org_user_id'),'status'=>"0","type"=>"0"]);
                    //插入标签
                    if ($label1){
...
//            $data_nn = Db::table("labels")->field("l_name as label,l_id as value,'0' AS type")->where("l_name","=",$query)->select();
    ...
    public static function index_importquery()
    {
        ...
        if ($query == "")
            return ['type'=>false,"info"=>"未获取到输入的标签!"];
        $data = Db::table("labels_school")->field("l_name as label,s_id as value")->where("l_name","=",$query)->select();
        if (empty($data)){
            //远程获取
